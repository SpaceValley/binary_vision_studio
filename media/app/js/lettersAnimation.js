(function() {
  ...
  const careerHeight =  document.querySelector(".designs").getBoundingClientRect().top + window.pageYOffset
  let recomposed = false

  const recomposeLetters = () => {
    [...document.querySelectorAll(".designs__title span")].forEach(letter => {
  ...
  const spreadLetters = () => {
    [...document.querySelectorAll(".designs__title span")].forEach((letter, i) => {
      const posX = i % 2 === 0 ? -(i + 10) * Math.random() * 7 : (i + 10) * Math.random() * 7
      const posY = i % 2 === 0 ? (i + 10) * Math.random() * 7 : -(i + 10) * Math.random() * 7
      letter.style.opacity = "0" 
      letter.style.transition = "all 1s cubic-bezier(0.88, 0.26, 0.3, 0.98)"
